...
- [x] **Love Reading Spread (Session 9)**: Complete 3-card love reading — partner name input, floating hearts animation (15 particles), sparkle burst on flip (20 particles), pulsing heart, love meter bar, pink/red romantic theme, 3D preview modal. Fortune-teller tone. 10-coin cost.
- [x] **Career & Money Reading Spread (Session 9)**: Complete 3-card career reading with 3-stage flow (SETUP → DIAGNOSTIC → READING). Diagnostic: 3 required tap-option questions (work situation, financial stress, internal obstacle) + 1 optional sanitized text input. Cards: Current Position / Hidden Block / Opportunity Window. Mentor tone (calm, direct, no mysticism). Professional blue theme. Input sanitization against prompt injection. 12-coin cost (`CAREER_SPREAD_COST`). Per-spread cost in Tarot.tsx.
- [x] **Shadow Energy System (Session 9)**: Complete psychological shadow work tarot system — the most premium feature. Multi-stage flow (ENTRY → Q1 → Q2 → MAIN_CARD → INTERPRETATION → SECRET → INTEGRATION + next-day REFLECTION). Weighted card draw (70% random / 30% keyword-influenced). Intensity scoring (1-10) with conditional secret card (≥7 always, 5-6 → 30%). Shadow mentor tone (calm, insightful, psychological, no mysticism). Dark minimal design (#050510 bg, floating shadow motes). 6 GPT functions. Next-day reflection system (2 coins). Dynamic pricing: first session 12 coins (`SHADOW_FIRST_COST`), subsequent 18 coins (`SHADOW_SESSION_COST`), reflection 2 coins (`SHADOW_REFLECTION_COST`). 3D card preview, session cache/restore, loading animations, interstitial ad on exit. ~35 new i18n keys across 7 locales. New screen `ShadowSession.tsx` (~800+ lines).
- [x] **Deep Reflection System (Session 9)**: Premium AI-guided clarity session (NOT a tarot spread). Chat-style interface. 4-stage flow: ENTRY → INPUT (up to 1500 chars) → CHAT (chat bubbles with typewriter effect) → CLOSED. 4 action types: Go Deeper / Reveal a Card / Examine Your Role / See Their Energy. Economy: 25 coins entry (`DEEP_REF_SESSION_COST`), 5 coins per action (`DEEP_REF_ACTION_COST`). `callOpenAIChat()` for multi-message GPT conversations with full history. Weighted tarot card draw (50% context-influenced). Session permanently closes on exit/back/60s background — no resume. Soft limit at 10 actions. Premium dark design (#040408 bg), staggered entry animations, ink line decorations, typed AI responses, coin deduction badges, thinking indicators. 3 GPT functions + `DEEP_REF_SYSTEM_PROMPT`. 18 i18n keys across 7 locales. Screen type `DEEP_REFLECTION` (now 18 total).
- [x] **RewardedAdModal Bug Fix (Session 9)**: Fixed `Objects are not valid as a React child` crash — error objects now extract `.message` string.
...
- [x] **Loading Animations (Session 9)**: `CardLoadingOverlay` (pulsing glow on card) + `InterpretingShimmer` (skeleton lines + animated dots) in `components/CardLoadingOverlay.tsx`. Applied to all spread screens with per-spread color theming.
- [x] **Interstitial Ad on Reading Exit (Session 9)**: `services/admobInterstitial.ts` with `showReadingExitAd(isPremium, onDone)` — 6s timeout fallback. Wired to exit buttons of all spread screens.
- [x] **Lucky Numbers 2 Free + 3 Locked (Session 9)**: Changed from 1 free + 4 locked to 2 free + 3 locked in Dashboard.tsx.
